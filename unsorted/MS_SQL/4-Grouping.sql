...
/*
 ...
 -> imagine each row as an individual and you group them together into teams, those teams can be selected and filtered based
    on conditions that apply to the team as a whole
*/

SELECT * FROM production.products;

-- group by for model year (doesn't tell us much, b/c only access model year)
SELECT 
    model_year 
FROM 
    production.products 
...
-- use aggreggate function count() -> count number of rows per group
...
-- can still use WHERE clause, needs be before group by, in order to filter the table before
-- it gets grouped
SELECT 
    model_year,
    COUNT(*) AS '# of products'
...
-- COUNT -> count number of rows that match a given citeria
-- AVG -> average value of a column (numeric) 
-- MAX -> get max value of a column
...
-- can do a normal select of our columns from student, but can also display aggregate evaluations

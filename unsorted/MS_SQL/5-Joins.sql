...
-- left table -> table listed first in query
-- right table -> table listed second in query

-- all records from left table (orders) are shown, will display nulls where no customer info
-- can be given
SELECT 
    *
FROM
...
-- get the product id, name, brand name, catagory name
SELECT 
    product_id,
    product_name,
    brand_name,
...
-- can add in order bys, group bys, where, etc.
SELECT 
    product_id,
    product_name,
    brand_name,
...
SELECT
    empl.first_name + ' ' + empl.last_name 'staff member',
    mngr.first_name + ' ' + mngr.last_name 'manager'
FROM 
    sales.staffs empl -- foreign key: manager_id
LEFT JOIN 
    sales.staffs mngr -- referenced foreign key: staff_id
ON 
    empl.manager_id = mngr.staff_id;

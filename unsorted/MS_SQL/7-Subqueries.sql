...
-- average list price for whole table
SELECT
    AVG(list_price)
FROM
    production.products;
...
-- inner query will have access to information from the outer query
...
--     prodouter.category_id,
--     prodouter.list_price,
--     AVG( prodouter.list_price) OVER (PARTITION BY prodouter.category_id) 'category avg'
-- FROM
--     production.products prodouter
-- WHERE
--     list_price > (
--         SELECT 
--             AVG(prodinner.list_price)
--         FROM 
--             production.products prodinner
--         WHERE
--             prodinner.category_id = prodouter.category_id
--     );
